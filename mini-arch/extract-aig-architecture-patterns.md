...
![DSL运算符使用分布](results/dsl_grammar.png)
...
![并发引擎架构](results/scan_engine.png)
...
![版本比较运算符分布](results/version_dsl.png)
...
![Go vs Python 任务分布](results/hybrid_arch.png)
...
- 静态检测规则（`rules` 字段）：正则 pattern 匹配代码中的危险模式
- LLM 提示词模板（`prompt_template` 字段）：指导 LLM 对匹配结果做深度推理

对 `data/mcp/` 目录下 4 条 MCP YAML 规则的分析显示，每条规则都包含 `prompt_template`，平均 prompt 长度约 3254 字符。
...
![Rule-First + LLM 流水线](results/rule_llm_layered.png)
...
对 `data/vuln/` 目录下 1691 条 CVE 规则的严重度分布进行统计后模拟评分，结果如下：

- **当前公式评分**：0（满分 100 被扣到 0）
- **CVSS 加权假设评分**：0（调整权重后仍然为 0）
- **比例制假设评分**：较高（按漏洞数量比例扣分）

核心问题在于：当漏洞数量足够多时（1691 条中约 61% 为 High/Critical），固定扣分制的总分会迅速触底为 0，失去区分能力。一个有 5 条 High 漏洞的目标和一个有 500 条 High 漏洞的目标，在当前算法下评分相同（都是 0）。
...
![评分算法对比](results/scoring_algorithm.png)
...
![部署形态](results/deployment_profiles.png)
