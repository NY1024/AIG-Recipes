...
**Screenshot-Augmented LLM Sensitivity Detection 模式**：三步流水线（截图→视觉分析→综合推理），视觉信息弥补了纯 HTML 分析无法理解 JavaScript 渲染内容的缺陷，结构化 prompt 通过角色设定+案例引导+内网上下文预设实现可控的风险判断。XML 标签输出便于程序解析。
