...
本文所有配套脚本位于 [mini-method/](https://github.com/NY1024/AIG-Recipes/tree/main/mini-method) 文件夹，可独立运行，只需 `git clone` AIG 仓库后搭配使用即可。研究者可以直接运行得到结构化的方法清单 JSON/CSV，也可以在此基础上修改筛选条件、提取更多元数据、对接自己的实验流程。
...
|---------|-------------|------|
| 工具滥用 | Yes (42 vectors) | Yes |
| 命令注入 | Yes (in tool-abuse) | Yes |
| 路径遍历 | Yes (13 vectors) | Yes |
| SSRF | Yes (in tool-abuse) | Yes |
| 授权绕过 | Yes (15 vectors) | Yes |
| 数据泄露 | Yes (3 vectors) | Yes |
| 提示注入 | Yes (22 vectors) | Yes |
| 记忆投毒 | Yes (9 vectors) | Yes |
| 硬编码密钥 | Yes (14 vectors) | Yes |
...
检测结果分 3 级：Pass - 未发现风险 / Warning - 需关注 / Risk - 发现风险。还附带 11 个示例 Skill 样本（含安全样本和 9 类风险样本），可直接作为评测集。
...
| 示例 Skill | 类别 | 预期检测结果 |
|-----------|------|-------------|
| risks-backdoor-persistence | 后门/持久化 | Risk - 发现风险 |
| risks-credential-exfil | 凭据外泄 | Risk - 发现风险 |
| risks-hardcoded-secret | 硬编码密钥 | Risk - 发现风险 |
| risks-jailbreak-encoded | 编码越狱 | Risk - 发现风险 |
| risks-privacy-snoop | 隐私窃取 | Risk - 发现风险 |
| risks-reverse-shell | 反向 Shell | Risk - 发现风险 |
| risks-tool-tamper | 工具篡改 | Risk - 发现风险 |
| risks-unsafe-delete | 危险删除 | Risk - 发现风险 |
| needs-attention-net | 需关注 | Warning - 需关注 |
| safe-config-viewer | 安全 | Pass - 未发现风险 |
| safe-readme-helper | 安全 | Pass - 未发现风险 |
